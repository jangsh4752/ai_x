...
# 4. 서브 태그 {% block 블럭명 %} sodyd {% endblock %}
# 5. 주석 {% comment %} 내용 {% endomment %}
...
names_list=[] # post방식으로 넘어온 name을 append
...
def index():
    if request.method == 'GET':
        ...
        names_list.append(name.strip()) # post방식으로 넘어온 name을 append
        name_length = len(name) # post방식으로 넘어온 name의 길이 구하기
    price = 12000
